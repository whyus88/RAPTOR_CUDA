...
// --- MODIFIKASI: Constant Memory untuk Range Boundary ---
__constant__ uint64_t c_range_len[4];     // L = End - Start + 1
__constant__ uint64_t c_range_end[4];     // End
__constant__ uint64_t c_RangePointX[4];   // Koordinat X dari G * L
__constant__ uint64_t c_RangePointY[4];   // Koordinat Y dari G * L
__constant__ int c_random_mode;           // Flag 1 jika random, 0 jika linear
...
// Helper untuk membandingkan a >= b (unsigned 256-bit)
__device__ bool ge256_u64(const uint64_t a[4], const uint64_t b[4]) {
    // Bandingkan dari limb paling signifikan
...
)
{
    ...
    const int is_random = c_random_mode;
...
#pragma unroll
        ...
        // Update Scalar S
        {
            ...
            // --- MODIFIKASI: WRAP LOGIC ---
            // Jika mode random, cek apakah S melewati batas range (c_range_end)
            if (is_random) {
                if (ge256_u64(S, c_range_end)) {
                    // Jika melebihi, kurangkan dengan panjang range (L)
                    // S = S - L
                    sub256_with_borrow(S, c_range_len, S);
                    
                    // P = P - RangePoint
                    // P ada di x1, y1. RangePoint ada di c_RangePointX/Y
                    // Kita lakukan Point Subtraction di sini.
                    // P_new = P - RangePoint = P + (-RangePoint)
                    // -RangePoint = (RangeX, -RangeY)
                    
                    uint64_t neg_Ry[4];
                    ModNeg256(neg_Ry, c_RangePointY); // -Y mod P_CURVE
                    
                    // Hitung gradien lam = (y1 - (-Ry)) / (x1 - Rx)
                    uint64_t dy[4], dx[4], dx_inv[5];
                    
                    ModSub256(dy, y1, neg_Ry); // y1 - (-Ry) = y1 + Ry
                    ModSub256(dx, x1, c_RangePointX);
                    
                    // Inverse dx
                    #pragma unroll
                    for(int k=0;k<4;k++) dx_inv[k] = dx[k];
                    dx_inv[4] = 0;
                    _ModInv(dx_inv);
                    ...
                    // Calc new X
                    uint64_t x_new[4], t[4];
                    _ModSqr(x_new, lam);
                    ModSub256(x_new, x_new, x1);
                    ModSub256(x_new, x_new, c_RangePointX);
                    
                    // Calc new Y
                    ModSub256(t, x1, x_new);
                    _ModMult(t, t, lam);
                    ModSub256(y1, t, y1); // y_new = lam(x1-x_new) - y1
                    // Note: y1 diupdate di akhir
                    
                    #pragma unroll
                    for(int k=0;k<4;k++) x1[k] = x_new[k];
                }
            }
            // -----------------------------
...
int main(int argc, char** argv) {
    ...
    // ... [Validasi Vanity & Range sama seperti sebelumnya] ...
    ...
    add256_u64(range_len, 1ull, range_len); // L = End - Start + 1
    ...
    // ... [Setup CUDA & Threads sama seperti sebelumnya] ...
    ...
    if (threadsTotal == 0ull) { std::cerr << "Error: Could not determine valid thread count.\n"; return EXIT_FAILURE; }
    ...
    uint64_t h_stride_scalar[4] = {0,0,0,0}; 

    // Konfigurasi Constant Memory baru
    uint64_t h_range_len[4], h_range_end[4];
    for(int k=0;k<4;k++) h_range_len[k] = range_len[k];
    for(int k=0;k<4;k++) h_range_end[k] = range_end[k];
    
    // Tambah +1 ke end karena kita ingin S <= End (jika S=End, itu batas terakhir)
    // Namun range_len = End - Start + 1.
    // Jika S melewati End (S > End), maka wrap.
    // Di kode, kita compare S >= c_range_end.
    // Jadi c_range_end harusnya End + 1.
    uint64_t h_range_limit[4];
    add256_u64(h_range_end, 1ull, h_range_limit); // Limit = End + 1

    cudaMemcpyToSymbol(c_range_len, h_range_len, 4 * sizeof(uint64_t));
    cudaMemcpyToSymbol(c_range_end, h_range_limit, 4 * sizeof(uint64_t)); // Simpan Limit

    int h_random_mode = random_mode ? 1 : 0;
    cudaMemcpyToSymbol(c_random_mode, &h_random_mode, sizeof(int));

    if (random_mode) {
        std::cout << "======= MODE: STRIDED RANDOM BATCH SEARCH (RANGE BOUND) ========\n";
        std::cout << "Seed: " << std::hex << random_seed << std::dec << "\n";
        
        // 1. Generate Random Stride (R) yang valid
        // Agar wrapping sederhana, kita buat R < RangeLen / B
        // Sehingga lompatan J = B*R < RangeLen.
        
        // Hitung MaxStride = RangeLen / B
        uint64_t max_stride[4];
        divmod_256_by_u64(range_len, (uint64_t)B, max_stride, r_u64); 
        
        std::uniform_int_distribution<uint64_t> dis64(1, UINT64_MAX);
        
        // Generate R < MaxStride
        // Untuk simplisitas, kita generate random 256-bit lalu modulo MaxStride
        // (Catatan: modular reduction 256-bit manual complex, disini kita lakukan approx untuk high bits jika perlu
        // atau gunakan RNG library jika support 128-bit/256-bit. 
        // Karena kode ini edukatif, kita asumsikan MaxStride muat dalam u64 atau kita handle simple case)
        
        // Asumsi sederhana: ambil 64-bit rendah dari MaxStride untuk RNG
        uint64_t ms_low = max_stride[0];
        if (max_stride[1] > 0 || max_stride[2] > 0 || max_stride[3] > 0) {
            // Jika range sangat besar, gunakan full random
             h_stride_scalar[0] = dis64(rng); h_stride_scalar[1] = dis64(rng); h_stride_scalar[2] = dis64(rng); h_stride_scalar[3] = dis64(rng);
             // Pastikan ganjil
             h_stride_scalar[0] |= 1ULL;
        } else {
            // Range cukup kecil
            if (ms_low > 1) {
                std::uniform_int_distribution<uint64_t> dis_stride(1, ms_low - 1);
                h_stride_scalar[0] = dis_stride(rng) | 1ULL; // Pastikan ganjil
            } else {
                h_stride_scalar[0] = 1;
            }
        }
        ...
        // 2. Compute Batch Deltas
        uint64_t current_acc[4] = {0,0,0,0};
        for (uint32_t i = 0; i < half; ++i) {
            add256_host(current_acc, h_stride_scalar, current_acc);
    ...
    // 5. Hitung Titik Jump (J) dan Range Point
    ...
    // Hitung Range Point (G * L)
    if (random_mode) {
        uint64_t *d_range_len, *d_temp_RPx, *d_temp_RPy;
        cudaMalloc(&d_range_len, 4 * sizeof(uint64_t));
        cudaMalloc(&d_temp_RPx, 4 * sizeof(uint64_t));
        cudaMalloc(&d_temp_RPy, 4 * sizeof(uint64_t));
        
        cudaMemcpy(d_range_len, h_range_len, 4 * sizeof(uint64_t), cudaMemcpyHostToDevice);
        scalarMulKernelBase<<<1, 1>>>(d_range_len, d_temp_RPx, d_temp_RPy, 1);
        cudaDeviceSynchronize();

        uint64_t h_RPx[4], h_RPy[4];
        cudaMemcpy(h_RPx, d_temp_RPx, 4 * sizeof(uint64_t), cudaMemcpyDeviceToHost);
        cudaMemcpy(h_RPy, d_temp_RPy, 4 * sizeof(uint64_t), cudaMemcpyDeviceToHost);

        cudaMemcpyToSymbol(c_RangePointX, h_RPx, 4 * sizeof(uint64_t));
        cudaMemcpyToSymbol(c_RangePointY, h_RPy, 4 * sizeof(uint64_t));

        cudaFree(d_range_len); cudaFree(d_temp_RPx); cudaFree(d_temp_RPy);
    }
    ...
    // ... [Inisialisasi Scalar & Main Loop selanjutnya tetap sama] ...
    // ...
